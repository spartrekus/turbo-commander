...
void ndata_draw()
{
  ...
  mvprintw( 0 , 0, "|NDATA|" );
  attroff( A_REVERSE );
  if ( fexist( thefilename )  == 1 ) ndata_print();
  attron( A_REVERSE );
  gfxhline( rows-1 );
...
int main( int argc, char *argv[])
{
    ...
    //strncpy( thefilename, "ndata.csv" , PATH_MAX );
